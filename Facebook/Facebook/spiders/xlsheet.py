...
class xlsfile(object):
	...
	self.excel_file_name = 'fullcontact_facebook3.xlsx'
    ...
    def send_xls(self):
		...
		print sk1
	        ...
	        for id_ls in id_list:
	            id1 = json.loads(id_ls[0]).get('sno','')
		    #if id1: id1=id1
		    #else: id1 = ''"""
		...
		if sno_f1:
			...
			sno_f = json.loads(sno_f1[0][0]).get('sno', '')
			payer = json.loads(sno_f1[0][0]).get('payer', '')
			print sno_f
		else:
			sno_f = ''
		if sno_f =='3431':import pdb;pdb.set_trace()
		else: pass
		...
		#aux_infof = json.loads(aux_info1)
	    	...
	    	aux_info_life_events = json.loads(aux_info_life_events.replace('\\','').replace('\n\n',''))
	    	aux_info_quotes = json.loads(aux_info_quotes.replace('\\','').replace('\n\n',''))
	    	aux_info_about = json.loads(aux_info_about.replace('\\','').replace('\r\n', '\\r\\n').replace('\t',''))
	    	aux_info_lived_places = json.loads(aux_info_lived_places.replace('\\','').replace('\n\n',''))
